...
public static class DonaldLandUtils 
{ 
  public static ObjRec[] getBlocks(int tileId)
  {
      ...
      {
          bb[i].palBytes[0] = (Globals.romdata[palAddr + i*2] & 0x30) >> 4;
      }
      return bb;
  }
  ...
  public static void setBlocks(int tileId, ObjRec[] blocksData)
  {
    ...
    for (int i = 0; i < count; i++)
    {
        int t = Globals.romdata[palAddr + i*2];
        t =  t &  0xCF | (blocksData[i].palBytes[0] << 4);
        Globals.romdata[palAddr + i*2] = (byte)t;
    }
  }
}